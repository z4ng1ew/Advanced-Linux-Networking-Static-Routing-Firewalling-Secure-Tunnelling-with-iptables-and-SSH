...
📍 **Сделать так, чтобы набрав `http://localhost:8080` на `ws21`, ты попал на `Apache`, работающий на `ws22`.**

---

## 📦 Что именно нужно сделать

### 1. Убедись, что Apache запущен на `ws22`
...
Если видишь что-то вроде `It works!` — значит Apache работает и слушает 80-й порт.

---

### 2. С `ws21` открой SSH-туннель к `ws22`
...
Введи пароль `ws22`, и **не закрывай это окно**, пока туннель используется.
