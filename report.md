...
### **Скриншот 1. Установка Apache2 на машины r1, r2 и ws22**
...
### **Скриншот 2. Изменение файла ports.conf на ws22 и r1**
...
### **Скриншот 3. Запуск Apache-сервера на ws22 и r1**
💬 *Запуск службы Apache с помощью команды `service apache2 start`. Успешный запуск подтверждает корректную установку и конфигурацию веб-сервера.*

---

### **Скриншот 4. Очистка iptables на r2 и установка политики по умолчанию**
...
### **Скриншот 5. Проверка недоступности ping между r1 и ws22**
...
### **Скриншот 6. Добавление разрешающего правила для ICMP**
...
### **Скриншот 7. Повторная проверка соединения после разрешения ICMP**
...
### **Скриншот 8. Настройка SNAT — маскирование внутренней сети**
💬 *Добавлено правило SNAT, маскирующее внутренние IP-адреса сети 10.20.0.0/24 за внешним IP маршрутизатора r2. Это позволяет устройствам из внутренней сети выходить в Интернет, используя внешний IP r2.*

---

### **Скриншот 9. Настройка DNAT для проброса порта 8080 на ws22**
...
### **Скриншот 10. Проверка SNAT с помощью telnet и tcpdump**
💬 *С машины ws22 устанавливается TCP-соединение с r1 по порту 80. С помощью `tcpdump` на r1 видно, что исходный адрес запроса — это IP маршрутизатора r2, что подтверждает корректную работу SNAT.*

---

### **Скриншот 11. Проверка DNAT — доступ к Apache на ws22 через r2**
💬 *С r1 производится подключение к r2 на порт 8080. На ws22 открыт порт 80, и благодаря правилу DNAT происходит перенаправление запроса, Apache корректно отвечает. Это демонстрирует работу проброса портов.*

---

### **Скриншот 12. Прослушка интерфейса enp0s8 на ws22 с tcpdump**
💬 *Для мониторинга сетевого трафика на ws22 используется `tcpdump` на интерфейсе `enp0s8`. Видно, что трафик действительно приходит на Apache по DNAT-правилу. Это финальное доказательство корректной настройки NAT.*
